...
void mems_finish() {
    ...
    mcNode *c = first;
    int getNo_of_main_nodes = last->mchainNo;
    mcNode *ptr[getNo_of_main_nodes];
    int i = 0;
    while (c != NULL && i < getNo_of_main_nodes) {
        ptr[i] = c;
        c = c->next;
        i++;
    }
    for (int i = 0; i < getNo_of_main_nodes; i++) {
        munmap(ptr[i]->mems_physical_address, ptr[i]->size);
    }
